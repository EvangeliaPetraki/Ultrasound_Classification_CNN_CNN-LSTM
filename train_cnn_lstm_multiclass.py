...
def unfreeze_layers(model, epoch, schedule):
    ...
    if epoch in schedule:
        ...
        cnn_layers = list(model.cnn.children())  # Get CNN layers
        
        for i in range(num_layers_to_unfreeze):
            for param in model.cnn.resnet.encoder.stages[-(i+1)].parameters():
                param.requires_grad = True
        print(f"Unfroze {num_layers_to_unfreeze} CNN layers at epoch {epoch}")

# Example unfreezing schedule
unfreeze_schedule = {5: 1, 10: 2, 15: 3}  # Unfreeze 1 layer at epoch 5, 2 layers at 10, etc.
...
if __name__ == '__main__':
    ...
    train_dataset = AlbumentationsDataset(TRAIN_DATASET, transform=trainTransform)
    # print('Training classes order: ', train_dataset.classes)

    test_dataset = AlbumentationsDataset(TEST_DATASET, transform=valTransform)
    ...
    network_backbone.load_state_dict(torch.load("/home/f/fratzeska/E/LSTM/2010869-intermediateep30.pth", map_location=torch.device(device))) 


    # network_backbone.load_state_dict(torch.load("C:/Users/Evangelia/Documents/Studies/medical engineering/TERM 2/Thesis/experiments/fine_tuned.pth", map_location=torch.device('cpu')))
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)%5 ==0:
            ...
            if os.path.exists(interm_path):
                ...
                print(f"Deleted intermediate model: {interm_path}", flush=True)
            else:
                print(f"Intermediate model file not found for deletion: {interm_path}", flush=True) 
